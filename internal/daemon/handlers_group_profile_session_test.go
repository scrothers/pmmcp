...
// TestDoRunGeneratesNameWhenOmitted pins the documented contract that --name
// is optional for one-shot runs: the daemon derives a unique run-… name so
// doStart's name-required invariant holds without agents inventing identities.
func TestDoRunGeneratesNameWhenOmitted(t *testing.T) {
	t.Parallel()
	c, _ := newTestDaemon(t, func(cfg *config.Config) { cfg.Sandbox.Default = "off" })
	c.SetSession("sess-full", "full")
	ctx := context.Background()
	var out api.StartResult
	if err := c.Call(ctx, api.MethodRun, api.RunPayload{
		StartPayload: api.StartPayload{Command: []string{"true"}, Sandbox: "off"},
	}, &out); err != nil {
		t.Fatalf("run without name: %v", err)
	}
	if !strings.HasPrefix(out.Name, "run-") || len(out.Name) != len("run-")+8 {
		t.Fatalf("autogenerated name = %q, want run-<8 hex chars>", out.Name)
	}
}
